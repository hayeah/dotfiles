    ...
    *,
    ...
    for chunk in _chunk_text(text):
        ...
        if not resp.is_success:
            log.error("sendMessage failed", status=resp.status_code, body=resp.text[:200])
...
def send_file(client: httpx.Client, token: str, chat_id: str, path: Path, caption: str | None) -> None:
    ...
    with open(path, "rb") as f:
        ...
        if not resp.is_success:
            log.error("send_file failed", method=method, status=resp.status_code, body=resp.text[:200])
        resp.raise_for_status()


@app.callback()
def send(
    message: list[str] = typer.Option([], "-m", help="Message lines (repeatable, joined with newlines)"),
    input_files: list[Path] = typer.Option([], "-i", help="Files to send (text pasted as message, images as photos)"),
    ...
    # When a subcommand is invoked, skip the default send behavior
    ctx = typer.Context
