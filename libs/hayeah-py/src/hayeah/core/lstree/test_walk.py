...
that reads JSON fixtures from ``libs/testdata/lstree.json``.
"""
...
from hayeah.core.lstree import Entry, Query, walk

# libs/hayeah-py/src/hayeah/core/lstree -> libs/
...
_FIXTURE_PATH = _LIBS_ROOT / "testdata" / "lstree.json"
_FIXTURES = (
    json.loads(_FIXTURE_PATH.read_text()) if _FIXTURE_PATH.exists() else None
)
...
def test_fixture_cases(tmp_path: Path, case: dict) -> None:
    ...
    got = [
        {"path": e.path.as_posix(), "isDir": e.is_dir}
        for e in entries
    ]
    expected = [
