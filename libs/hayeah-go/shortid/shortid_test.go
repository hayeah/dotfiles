...
// Verify expected types are serialized correctly
func TestTestDataTypes(t *testing.T) {
	// Verify we loaded valid test data by checking a few key fields
	raw, err := os.ReadFile("../../testdata/shortid.json")
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, ok := data["resolve_tests"]; !ok {
		t.Fatal("missing resolve_tests in testdata")
	}
}
